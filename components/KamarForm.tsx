...
import { PlusCircle, Trash2, Upload, X, GripVertical } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
import type { KamarWithHarga, KamarStatus } from "@/types";
...
export function KamarForm({ kamar }: Props) {
  ...
  function addCustomFasilitas() {
    ...
    if (trimmed && !fasilitas.includes(trimmed)) {
      setFasilitas((prev) => [...prev, trimmed]);
    }
    setFasilitasCustom("");
  }
  ...
  async function removeFoto(url: string) {
    ...
    if (path) {
      await supabase.storage.from("foto-kamar").remove([path]);
    }
  ...
  function addHarga() {
    setHargaList((prev) => [
      ...prev,
      { durasi: "", harga: "", keterangan: "", urutan: prev.length },
    ]);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    // Sync harga: delete old, insert new
    if (kamarId) {
      ...
      const hargaPayload = hargaList
        ...
        .map((h, i) => ({
          kamar_id: kamarId!,
          durasi: h.durasi,
          harga: parseInt(h.harga.replace(/\D/g, "")),
  ...
  async function handleDelete() {
    if (!kamar?.id) return;
    if (!confirm(`Hapus kamar "${kamar.nama}"? Aksi ini tidak bisa dibatalkan.`)) return;
    setDeleting(true);
    ...
    <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition"
            />
          </div>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition resize-none"
            />
          </div>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
        ...
        <div className="flex gap-2">
          <input
            value={fasilitasCustom}
            onChange={(e) => setFasilitasCustom(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addCustomFasilitas(); } }}
            placeholder="Tambah fasilitas lain..."
            className="flex-1 rounded-xl border border-gray-200 px-4 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition"
          />
          <button
            type="button"
            onClick={addCustomFasilitas}
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
        ...
        <div className="mb-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
          ...
          <button
            ...
            disabled={uploading}
            className="flex aspect-square flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-gray-200 text-gray-400 hover:border-brand-400 hover:text-brand-500 transition disabled:opacity-50"
          >
          ...
          {hargaList.map((h, idx) => (
            <div key={idx} className="flex items-start gap-2">
              <div className="grid flex-1 gap-2 sm:grid-cols-3">
                <input
                  value={h.durasi}
                  onChange={(e) => updateHarga(idx, "durasi", e.target.value)}
                  placeholder="Durasi (e.g. Bulanan)"
                  className="rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition"
                />
                <input
                  value={h.harga}
                  onChange={(e) => updateHarga(idx, "harga", e.target.value)}
                  placeholder="Harga (e.g. 1500000)"
                  className="rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 transition"
                />
                <input
                  value={h.keterangan}
                  onChange={(e) => updateHarga(idx, "keterangan", e.target.value)}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <button
            type="submit"
            disabled={saving}
            className="rounded-xl bg-brand-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-60 transition"
          >
